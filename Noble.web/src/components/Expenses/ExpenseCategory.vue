<template>
    <div class="row">
        <div class="col-lg-12">
                            ...
                            <tbody>
                                <tr v-for="(brand, index) in authorizedPersonlist" v-bind:key="brand.id">
                                    ...
                                    <td class="text-center">
                                        <strong>
                                            <a href="javascript:void(0)" v-on:click="EditauthorizedPerson(brand.id)">{{ brand.expenseCategoryCode }}</a>
                                        </strong>
                                    </td>
                                    <td class="text-start">
                                        <strong>
                                            <a href="javascript:void(0)" v-on:click="EditauthorizedPerson(brand.id)"> {{ brand.categoryName }}</a>
                                        </strong>
                                    </td>
                                    <td class="text-center">
                                        {{ brand.description }}
            ...
            <expensecategory-mod :brand="newexpensecategory" :show="show" v-if="show" @close="IsSave" :type="type" />
        </div>
...
export default {
    ...
    methods: {
        ...
        GetauthorizedPerson: function () {
            ...
            root.$https.get('Benificary/GetExpenseCategoryList?pageNumber=' + this.currentPage + '&searchTerm=' + this.search, { headers: { "Authorization": `Bearer ${token}` } }).then(function (response) {
        ...
        EditauthorizedPerson: function (Id) {
            ...
            root.$https.get('/Benificary/GetExpenseCategoryDetail?Id=' + Id, { headers: { "Authorization": `Bearer ${token}` } })
                .then(function (response) {
                    if (response.data) {
                        root.newexpensecategory= response.data;
                        root.show = !root.show;
                        root.type = "Edit"
                    } else {
