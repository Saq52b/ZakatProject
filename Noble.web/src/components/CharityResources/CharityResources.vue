                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table mb-0">
                            ...
                            <tbody>
                                <tr v-for="(brand, index) in charityResourceslist" v-bind:key="brand.id">
                    ...
                    <div class="float-end">
                        <div class="" v-on:click="GetCharityResourcesData()">
...
export default {
    ...
    watch: {
        search: function (val) {
            this.GetCharityResourcesData(val, 1);
        }
    },
    methods: {
        IsSave: function () {
            this.show = false;
            this.GetCharityResourcesData(this.search, this.currentPage);
        },
        ...
        GetCharityResourcesData: function () {
            var root = this;
            var token = '';
            if (this.$session.exists()) {
                token = localStorage.getItem('token');
            }
            root.$https.get('Benificary/GetCharityResourcesList?pageNumber=' + this.currentPage + '&searchTerm=' + this.search, { headers: { "Authorization": `Bearer ${token}` } }).then(function (response) {
                if (response.data != null) {
                    root.charityResourceslist = response.data.results;
        ...
        EditCharityResources: function (Id) {
            ...
            root.$https.get('/Benificary/GetCharityResourcesDetail?Id=' + Id, { headers: { "Authorization": `Bearer ${token}` } })
                .then(function (response) {
                    if (response.data) {
                        root.newCharityResources= response.data;
                        root.show = !root.show;
                        root.type = "Edit"
                    } else {
    ...
    mounted: function () {
        ...
        this.GetCharityResourcesData(this.search, 1);
