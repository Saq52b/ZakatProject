<template>
    <div class="row">
        <div class="col-lg-12">
            <div class="row">
                <div class="col-sm-12">
                    <div class="page-title-box">
                        <div class="row">
                            <div class="col">
                                <h4 class="page-title">{{ $t('PaymentType.PaymentType') }}</h4>
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="javascript:void(0);">{{ $t('Home') }}</a>
                                    </li>
                                    <li class="breadcrumb-item active">{{ $t('PaymentType.PaymentTypeList') }}</li>
                                </ol>
                            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="input-group">
                        ...
                        <input v-model="search" type="text" class="form-control" :placeholder="$t('PaymentType.SearchByName')"
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table mb-0">
                            ...
                            <tbody>
                                <tr v-for="(brand, index) in paymentTypelist" v-bind:key="brand.id">
                                    ...
                                    <td class="text-center" v-if="brand.name != ''">
                                        <strong>
                                            <a href="javascript:void(0)" v-on:click="EditpaymentTypelist(brand.id)"> {{ brand.name }}</a>
                                        </strong>
                                    </td>
                    ...
                    <div class="float-end">
                        <div class="" v-on:click="GetpaymentType()">
            ...
            <paymenttype-mod :brand="newpaymentType" :show="show" v-if="show" @close="IsSave" :type="type" />
        </div>
...
export default {
    ...
    watch: {
        search: function (val) {
            this.GetpaymentType(val, 1);
        }
    },
    methods: {
        IsSave: function () {
            this.show = false;
            this.GetpaymentType(this.search, this.currentPage);
        },
        ...
        GetpaymentType: function () {
            ...
            root.$https.get('Benificary/GetPaymentTypeList?pageNumber=' + this.currentPage + '&searchTerm=' + this.search, { headers: { "Authorization": `Bearer ${token}` } }).then(function (response) {
                if (response.data != null) {
                    root.paymentTypelist = response.data.results;
        ...
        EditpaymentTypelist: function (Id) {
            ...
            root.$https.get('/Benificary/GetPaymentTypeDetail?Id=' + Id, { headers: { "Authorization": `Bearer ${token}` } })
                .then(function (response) {
                    if (response.data) {
                        root.newpaymentType = response.data;
                        root.show = !root.show;
                        root.type = "Edit"
                    } else {
    ...
    mounted: function () {
        ...
        this.GetpaymentType(this.search, 1);
